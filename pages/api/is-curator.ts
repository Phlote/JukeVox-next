...
export default async function handler(
  ...
  const resp = await fetch(
    `https://api-testnet.polygonscan.com/api?module=account&action=tokenbalance&contractaddress=${PHOTE_VOTE_TOKEN_ADDRESS}&address=${wallet}&tag=latest&apikey=${process.env.POLYGONSCAN_API_KEY}`
  );
